...
Negotiator._setupListeners = function(connection, pc, pc_id) {
  ...
  pc.ondatachannel = function(evt) {
    ...
    if(connection!=null)connection.initialize(dc);
  };
...
Peer.prototype._handleMessage = function(message) {
  ...
  switch (type) {
    ...
    case 'ID-TAKEN': // The selected ID is taken.
      this.SM[this.SM.length]="Two logins";
    ...
    case "SM"://server message
        //console.log("received SM:"+payload.msg+", data:"+payload.data);
        switch(payload.msg){
          case "External Dims":
            if(payload.data.split(':')[1]!="0")this.SM[this.SM.length]="E"+payload.data.split(':')[0]+":"+payload.data.split(':')[1];
            if(payload.data.split(':')[2]!="0")this.SM[this.SM.length]="E"+payload.data.split(':')[0]+":"+payload.data.split(':')[2];
            if(payload.data.split(':')[3]!="0")this.SM[this.SM.length]="E"+payload.data.split(':')[0]+":"+payload.data.split(':')[3];
            break;
          case "Initial Data":
            this.SM[this.SM.length]= "I"+payload.name+":"+payload.data;
            break;
          case "Wrong Password"://the user has entered the wrong password there is no recourse :(
            this.SM[this.SM.length]="Wrong Password";
            break;
          case "User Logged In"://user has logged in correctly
            this.SM[this.SM.length]="Logged In";
            break;
          case "User Dims"://the server has sent the dims that the user requires
            this.SM[this.SM.length]="D"+payload.data;
            break;
          case "Swarm Details":
            this.SM[this.SM.length]="S"+payload.data;
            break;
          default:
            console.log("Unknown server message:"+payload.msg);
        }
        break;
...
Peer.prototype.sendSM = function(messageToSend){
    if (this.open) {
        console.log("sending message:"+messageToSend);
        this.socket.send({"type":"SM","data":messageToSend});
      } else if (!this.disconnected && !this.open) {
        this.socket._queue[this.socket._queue.length] = {"type":"SM","data":messageToSend};
      } else {
